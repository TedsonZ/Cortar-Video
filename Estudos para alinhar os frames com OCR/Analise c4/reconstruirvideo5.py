...
def load_timestamps(csv_path):
    ...
    # Ordena por timestamp e número do frame
...
def fill_missing_frames(timestamps):
    ...
    for second, frames in sorted(per_second.items()):
        if len(frames) < (FPS - 1):  # nova tolerância: só preenche se tiver menos de 29
...
def gerar_video_ffmpeg(frame_list, output_path):
    ...
    # Corrigido: usar -r no final, não -framerate com concat
